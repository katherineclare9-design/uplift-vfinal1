...
const todaysEvents = userData.calendarEvents.filter(event =>event.date === today);
...
${userData.streak < 3?"🌱 3 Days":userData.streak < 7?"🔥 7 Days":userData.streak < 14?"⚡ 14 Days":userData.streak < 30?"🏆 30 Days":"👑 100 Days"}
...
userData.selectedDiaryDate ||new Date().toLocaleDateString();
...
<!-- Regular Morning removed in Event Mode -->



${[].map(item=>`

<p>Optional regular workout available</p>
...
const earned =userData.unlockedBadges.length;
...
const selected =userData.selectedBadgeCategory || "Getting Started";
...
<buttonclass="${selected===category ? "active-tab" : ""}"onclick="changeBadgeCategory('${category}')">
...
const unlocked =userData.unlockedBadges.includes(id);
